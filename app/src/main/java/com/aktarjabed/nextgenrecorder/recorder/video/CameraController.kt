...
private var faceAutoFramer: FaceAutoFramer? = null
...
fun enableFaceDetection(enable: Boolean, lifecycleOwner: LifecycleOwner) {
    if (enable && faceAutoFramer == null) {
        faceAutoFramer = FaceAutoFramer(context)

        // Setup ImageAnalysis for face detection
        imageAnalysis = ImageAnalysis.Builder()
            .setBackpressureStrategy(ImageAnalysis.STRATEGY_KEEP_ONLY_LATEST)
            .build()
            .also { analysis ->
                analysis.setAnalyzer(ContextCompat.getMainExecutor(context)) { imageProxy ->
                    processFrameForFaceDetection(imageProxy)
                }
            }

        // Bind image analysis to camera
...
private fun processFrameForFaceDetection(imageProxy: ImageProxy) {
    val image = imageProxy.image
    if (image != null) {
        val bitmap = imageToBitmap(image)
        faceAutoFramer?.processFrame(bitmap, camera?.cameraControl, camera?.cameraInfo)
    }
    imageProxy.close()
}
